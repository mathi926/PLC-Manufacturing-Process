...
readings.to_excel("fact_readings.xlsx", index=False)
...
downtime.to_excel("fault_events.xlsx", index=False)
...
print("\nFiles saved: fact_readings.xlsx, fault_events.xlsx, kpi_summary.xlsx M:/Project")

with pd.ExcelWriter("M:/Project/manufacturing_report.xlsx",engine="openpyxl")as writer:
    kpi_summary.to_excel(writer,sheet_name="KPI summary",index=False)
    downtime.to_excel(writer,sheet_name="fault Events",index=False)
    readings.to_excel(writer,sheet_name="fact Readings",index=False)

print("Excel file saved :M:/Project/manufacturing_report.xlsx")
